...
class CreateNpcEventCronjob implements CronjobTaskInterface
{
    function run()
    {
        global $db;

        // Vérifier si déjà un NPC aujourd'hui
        $today = date('Y-m-d');
        ...
        for ($i = 0; $i < 1000; $i++) {
            ...
            $pos = $db->uniquequery("SELECT COUNT(*) as count FROM uni1_planets WHERE galaxy = $galaxy AND system = $system AND planet = $planet");
